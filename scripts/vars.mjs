...
// Self-maintained fork: every build off this branch is versioned <nearest upstream tag>γ.
// γ isn't valid semver, so the package version is X.Y.Z-gamma and the UI maps it back to γ
// (see getAppVersion in tabby-electron)
export const forkSuffix = 'γ'
export const forkPrerelease = 'gamma'

export let version = childProcess.execSync('git describe --tags', { encoding:'utf-8' })
version = version.substring(1).trim().replace(forkSuffix, '').split('-')[0]
if (!semver.valid(version)) {
    throw new Error(`Could not derive a version from git describe: ${version}`)
}
version = `${version}-${forkPrerelease}`
